...
const createAddsale = async (addsaleData) => {
  try {
    const result = await api.post('/products', addsaleData)
    return result.data
  } catch (error) {
    throw new Error(error.message)
  }
...
const uploadImageUrl = async (image) => {
  try {
    const result = await api.patch('products/image', image)
...
const getAddsalesByUser = async (userId) => {
  try {
    const result = await api.get(`/products?user_id=${userId}`)
    return result.data
  } catch (error) {
    throw new Error(error.message)
  }
